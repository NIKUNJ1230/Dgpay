...
const CreditCard = () => {
  ...
  const toggleModal = async () => {
    try {
      const response = await axios.post(
        `${Allapi.port}${Allapi.add_money_otp}`,
  ...
  const handleVerifyOTP = async () => {
    ...
    if (enteredOTP.length === 6) {
      ...
      // If OTP is not complete, show an error message or perform any other action as needed
      console.log("Incomplete OTP. Please enter 6 digits.");
    }
  };
